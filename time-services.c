...
enum time_service getServiceForString(const char* aServiceString) {
    if (strcmp("JJY", aServiceString) == 0)
        return JJY;
    if (strcmp("JJY40", aServiceString) == 0)
        return JJY40;
    if (strcmp("JJY60", aServiceString) == 0)
        return JJY60;
    if (strcmp("WWVB", aServiceString) == 0)
        return WWVB;
    if (strcmp("DCF77", aServiceString) == 0)
        return DCF77;
    if (strcmp("MSF", aServiceString) == 0)
        return MSF;
		if (strcmp("LEGACY", aServiceString) == 0)
				return LEGACY;
    return WWVB;
}

const char* getStringForService(enum time_service aService) { 
    if (aService == JJY)
        return "JJY";
    if (aService == JJY40)
        return "JJY40";
...
int getFrequencyForService(enum time_service aTimeService) {
    switch (aTimeService) {
        case JJY:
        case JJY40:
            return 40000;
...
bool getLogicForMillisecond(enum time_service service, int modulation, int millis) {
		switch (service) {
			case JJY:
			case JJY40:
			case JJY60:
				return millis <= modulation;
			default:
				return millis >= modulation;
		}
}